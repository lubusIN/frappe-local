...
import { orchestrateBenchStop } from '../src/main/bench-orchestration';
...
describe('bench stop orchestration', () => {
  ...
  it('uses bench stop timeout configuration', async () => {
    ...
    expect(args).toEqual(['-p', 'frappe-cafe-1adb2eed', 'stop', '--timeout', '20']);
  ...
  it('falls back to down when stop command times out', async () => {
    ...
    expect(firstArgs).toEqual(['-p', 'frappe-cafe-1adb2eed', 'stop', '--timeout', '20']);
    expect(secondArgs).toEqual(['-p', 'frappe-cafe-1adb2eed', 'down', '--remove-orphans', '--timeout', '20']);
  });
