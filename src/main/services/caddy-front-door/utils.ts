...
export const CADDY_RUNTIME_DIR = path.join(os.tmpdir(), 'frappe-local-caddy');
export const CADDY_CONFIG_PATH = path.join(CADDY_RUNTIME_DIR, 'Caddyfile');
export const CADDY_PID_PATH = path.join(CADDY_RUNTIME_DIR, 'caddy.pid');
...
export type FrontDoorRepositories = {
  readonly benches: {
...
export const canBindFrontDoorPort = (port: number): Promise<boolean> => new Promise((resolve) => {
...
export const ensureCaddyRootTrusted = async (): Promise<boolean> => {
...
export const canReuseManagedFrontDoor = (desiredConfig: string): boolean => {
  try {
...
export const stopOrphanFrontDoorProcesses = (): void => {
...
export const stopPidFromFile = (): void => {
  try {
    if (!fs.existsSync(CADDY_PID_PATH)) {
      return;
    }
...
export const buildFrontDoorRoutes = async (repositories: FrontDoorRepositories): Promise<FrontDoorRoute[]> => {
