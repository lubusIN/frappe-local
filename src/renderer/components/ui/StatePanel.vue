<template>
  <section
    class="flex items-start gap-2.5 rounded-6 border p-4 select-text"
    :class="{
      'border-outline-gray-2 bg-surface-base': kind === 'empty',
      'border-outline-gray-2 bg-surface-gray-2': kind === 'loading',
      'border-outline-red-3 bg-surface-red-2': kind === 'error',
    }"
    role="status"
    aria-live="polite"
    :aria-busy="kind === 'loading'"
    :aria-label="accessibilityLabel"
  >
    <div
      class="mt-0.5 min-w-4"
      :class="kind === 'error' ? 'text-ink-red-7' : 'text-ink-gray-5'"
    >
      <LoadingIndicator
        v-if="kind === 'loading'"
        class="h-4 w-4"
      />
      <i
        v-else-if="kind === 'empty'"
        class="lucide-inbox h-4 w-4"
      />
      <i
        v-else
        class="lucide-alert-triangle h-4 w-4"
      />
    </div>
    <div class="min-w-0">
      <h4
        class="m-0 text-[13px] font-semibold"
        :class="kind === 'error' ? 'text-ink-red-7' : 'text-ink-gray-9'"
      >
        {{ title }}
      </h4>
      <p
        v-if="body"
        class="mt-1 text-[13px] leading-relaxed"
        :class="kind === 'error' ? 'text-ink-red-7' : 'text-ink-gray-6'"
      >
        {{ body }}
      </p>
      <Button
        v-if="actionLabel"
        class="mt-2"
        :theme="kind === 'error' ? 'red' : 'gray'"
        variant="outline"
        :aria-label="actionAriaLabel"
        @click="$emit('action')"
      >
        {{ actionLabel }}
      </Button>
    </div>
  </section>
</template>

<script setup lang="ts">
import { Button, LoadingIndicator } from 'frappe-ui';
import { computed } from 'vue';

const props = defineProps<{
  kind: 'loading' | 'empty' | 'error';
  title: string;
  body?: string;
  actionLabel?: string;
}>();

defineEmits<{
  (event: 'action'): void;
}>();

const accessibilityLabel = computed(() => {
  if (props.kind === 'loading') return `Loading: ${props.title}`;
  if (props.kind === 'error') return `Error: ${props.title}`;
  return `Empty: ${props.title}`;
});

const actionAriaLabel = computed(() => {
  if (props.kind === 'error') return `${props.actionLabel}: retry this action`;
  if (props.kind === 'loading') return `${props.actionLabel}: cancel loading`;
  return `${props.actionLabel}`;
});
</script>
